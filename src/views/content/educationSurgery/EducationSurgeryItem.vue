<!--  -->
<template>
  <section class="education-surgery-item-section">
    <el-card class="education-surgery-card-box">
      <el-form :model="educationSurgery" class="education-surgery-form" ref="educationSurgeryForm"
        :rules="educationSurgeryRules" label-position="top">

        <div class="outer-input-box">

          <div class="text-input-box">
            <el-form-item label="文章類別" prop="type">
              <el-tree-select v-model="educationSurgery.categoryId" value-key="id" :data="categoryOption"
                :render-after-expand="false" style="width: 240px" :empty-values="[null, undefined]"
                :value-on-clear=null>
              </el-tree-select>
            </el-form-item>
            <el-form-item label="文章標題" prop="title">
              <el-input v-model="educationSurgery.title" placeholder="消息標題" />
            </el-form-item>
            <el-form-item label="文章描述">
              <el-input type="textarea" v-model="educationSurgery.description" autocomplete="off" />
            </el-form-item>
          </div>
          <div class="img-box">
        ...
        <div style="text-align: right;">
          ...
          <el-button type="primary" @click="submitEducationSurgeryForm(educationSurgeryForm)">保存</el-button>
        </div>
      </el-form>

      <CustomCKEditor :scope="scope" :htmlContent="educationSurgery.content" :updateContent="updateContent">
      </CustomCKEditor>
...
//數據,父傳子
const scope = ref("educationSurgeryScope")
 ...
 * 這邊將數組放入,將educationSurgeryCategoryId(PMI key)取出,賦予別名為id; name取出,賦予別名label方便後面通用組件顯示
 * 
 * @param arr 
 */
const transformArray = (arr: Record<string, any>[], primaryKeyName: string): Record<string, any>[] => {
  //在遍歷的每個item直接進行解構賦值,提取educationSurgeryCategoryId改名為id,以及其他屬性
...
//文章表單內容
const educationSurgery = reactive<Record<string, any>>({
  articleId: '',
  catrgoryId: '',
  type: '',
  title: '',
...
const updateContent = (newValue: string) => {
  educationSurgery.content = newValue;
};
...
//獲取新增最新消息表單DOM
const educationSurgeryForm = ref()
//表單校驗規則
const educationSurgeryRules = reactive({
  type: [
    {
      required: true,
      message: '類型不能為空',
...
//獲取文章
const getEducationSurgery = async () => {
  let res = await getArticleApi(props.id)
  Object.assign(educationSurgery, res.data)
}

//返回列表
const back = () => {
  router.push("/content/education-surgery")
}


//送出表單
const submitEducationSurgeryForm = (form: FormInstance) => {
  //沒有抓到的這個Dom直接返回
  if (!form) return
  form.validate(async (valid) => {
    if (valid) {
      ...
      //賦予進educationSurgery響應式對象中
      educationSurgery.tempUploadUrl = tempUploadUrlList

      let formData = new FormData();
      // 將響應式對象轉換為普通對象，然後轉換為 JSON 字符串
      const jsonData = JSON.stringify(educationSurgery)
...
onMounted(async () => {
  await getAllEducationSurgeryCategory()
  await getEducationSurgery()
  imageUrl.value = protocol + '//' + hostname + '/minio' + educationSurgery.coverThumbnailUrl
})
...
.education-surgery-item-section {
  width: 90%;
  margin: 1% auto;

  .education-surgery-card-box {
    min-height: 80vh;

    .education-surgery-form {
      margin-bottom: 1%;
